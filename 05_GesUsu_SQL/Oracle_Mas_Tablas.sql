...
INSERT INTO DEPARTAMENTO(NOMBRE, FECHA) VALUES ('Dirección', '20/1/2019');
...
SELECT * FROM usuario U JOIN DEPARTAMENTO
	ON u.DEPART_PRINCIPAL = DEPARTAMENTO.id;

SELECT * FROM usuario U JOIN DEPARTAMENTO D
	ON u.DEPART_PRINCIPAL = d.id WHERE D.Nombre = 'Marketing';
...
SELECT U.NOMBRE, DEPART from usuario u
	join USU_DEPT UD ON U.id = UD.ID.USU
	JOIN DEPARTAMENTO d ON UD.ID_DEPT = D.id
		WHERE d.nombre = 'Programación';

/* SELECT U.NOMBRE, DEPART from usuario u
	join USU_DEPT UD USING (U.id)
